...
public class DBManager{
    ...
    public static string conString=@"server=localhost;port=3306;user=root; password=password;database=transflower"; //register
    public  static List<Department> GetAllDepartments(){
            ...
            MySqlConnection con=new MySqlConnection(); //connecton object get connection
            con.ConnectionString=conString; //string set
            try{
                con.Open(); //open conenction
                //fire query to database
                MySqlCommand cmd=new MySqlCommand(); //command just like prepared statement
                cmd.Connection=con; //cmd object will have connection object and query
                string query="SELECT * FROM departments";
                cmd.CommandText=query; //adding query to the command object
                MySqlDataReader reader=cmd.ExecuteReader(); //reader just like result set
                //ExecuteScaler for aggregate functions
                while(reader.Read()){
                    ...
                    allDepartments.Add(dept); //adding to the list
                }
            }
            catch(Exception ee){
    ...
    public static Department GetDeparmentDetails(int id){
        ...
        try
        {
            ...
            con.Open();
            MySqlCommand command = new MySqlCommand(query, con); //adding query and connection at the same time
...
//Employee Operations CRUD
    public static bool DoesEmployeeExists(int id)
    {
        ...
        try
        {
            ...
            MySqlDataReader reader = cmd.ExecuteReader(); //For result set. only for ddl commands
            reader.Read();
